...
public class TfliteClassifier extends Classifier {
    ...
    public void loadModel(YoloModel yoloModel, boolean useGpu) throws Exception {
        if (yoloModel instanceof LocalModel) {
            ...
            if (localModel.modelPath == null || localModel.modelPath.isEmpty() ||
                    localModel.metadataPath == null || localModel.metadataPath.isEmpty()) {
                throw new Exception();
            }

            final AssetManager assetManager = context.getAssets();
            loadLabels(assetManager, localModel.metadataPath);
            try {
                MappedByteBuffer modelFile = loadModelFile(assetManager, localModel.modelPath);
    ...
    @Override
    public List<ClassifiedObject> predict(Bitmap bitmap) {
        try {
...
//            }
    ...
    public void predict(ImageProxy imageProxy, boolean isMirrored) {
        ...
        handler.post(() -> {
            ...
            List<ClassifiedObject> result = runInference();
    ...
    private List<ClassifiedObject> runInference() {
        List<ClassifiedObject> classifiedObjects = new ArrayList<>();
        ...
        if (interpreter != null) {
            ...
            if (byteBuffer != null) {
                ...
                classifiedObjects = list.stream()
                        ...
                        .map(map -> new ClassifiedObject((String) map.get("label"), (int) map.get("index"), (float) map.get("confidence")))
